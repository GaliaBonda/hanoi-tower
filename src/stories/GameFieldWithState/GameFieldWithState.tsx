...
function GameFieldWithState({ discsNumber = 3, popup, winPopup, stacks,
    closePopup, closeWinPopup, formStateStacks }: TestProps) {
    ...
    return (
        <>
            <StyledDiv >
                <GameInput value={discsNumber} handleChange={action('input changed')} formStacks={formStateStacks} />
                <StacksWrapper data-testid='test-stacks-wrapper' ref={rodsWrapperRef} onClick={action('click on game field')}>
                    {Object.values(stacks).map((item) => {
                        return (<Rod stack={item.stack} moveDisc={action('move disc')} id={item.id} key={item.id}
                            discsNum={stacksSize} />);
                    })}
                </StacksWrapper>
            </StyledDiv>
            {popup.isShown && <Popup title={popup.title} text={popup.text}
                closePopup={closePopup} gameControl={false} okLabel='Ok' cancelLabel='Cancel' />}
            {winPopup && <Popup title='Congratulations!!!&#127881;' text="Who's the winner? You are the winner! Would you dare to take another round?"
                closePopup={closeWinPopup} gameControl={true} okHandle={action('new game started')} okLabel='Ok' cancelLabel='Cancel' />}
        </>
